...
export const Tree: FC<TreeProps> = (props) => {
  const { items, selectedId, defaultExpandedIds, onSelect, icon, loadingFauxItems = 10, isLoading } = props;
  const { visibleItems, toggle, select, selected, expandedIds } = useTree(items, selectedId, defaultExpandedIds);
  ...
  if (isLoading) {
    return (
      <VBox gap="medium">
        {getSizedArray(loadingFauxItems, 0).map((_, index) => (
          <FauxItem key={index}>
            <Skeleton />
          </FauxItem>
        ))}
